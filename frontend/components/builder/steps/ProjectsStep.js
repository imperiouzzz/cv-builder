...
export default function ProjectsStep() {
  ...
  function addEntry() {
    setCVField("projects", [
      ...cv.projects,
      { id: uid(), name: "", tech: "", period: "", description: "", link: "" },
    ]);
  }
  function removeEntry(id) {
    setCVField(
      "projects",
      cv.projects.filter((e) => e.id !== id),
    );
  }
  function update(id, key, value) {
    setCVField(
      "projects",
      cv.projects.map((e) => (e.id === id ? { ...e, [key]: value } : e)),
    );
  }
  ...
  return (
    <>
      ...
      <Desc>
        Showcase academic projects, personal builds, or research that
        demonstrates your technical skills.
      </Desc>
      <TipBox>
        💡 ATS Tip: Include the exact technology names used (e.g. "TensorFlow",
        "Django REST Framework") — these are keywords recruiters search for.
      </TipBox>
