...
export default function ReferencesStep() {
  ...
  function addEntry() {
    setCVField("references", [
      ...cv.references,
      { id: uid(), name: "", title: "", org: "", email: "", phone: "" },
    ]);
  }
  function removeEntry(id) {
    setCVField(
      "references",
      cv.references.filter((e) => e.id !== id),
    );
  }
  function update(id, key, value) {
    setCVField(
      "references",
      cv.references.map((e) => (e.id === id ? { ...e, [key]: value } : e)),
    );
  }
  ...
  return (
    <>
      ...
      <Desc>
        Add professional references who can vouch for your skills and work
        ethic.
      </Desc>
      <TipBox>
        💡 Always ask permission before listing someone as a reference. Provide
        at least 2 references — one academic and one professional.
      </TipBox>
