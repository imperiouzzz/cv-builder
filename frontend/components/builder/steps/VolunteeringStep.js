...
export default function VolunteeringStep() {
  ...
  function addEntry() {
    setCVField("volunteering", [
      ...cv.volunteering,
      { id: uid(), role: "", org: "", period: "", desc: "" },
    ]);
  }
  function removeEntry(id) {
    setCVField(
      "volunteering",
      cv.volunteering.filter((e) => e.id !== id),
    );
  }
  function update(id, key, value) {
    setCVField(
      "volunteering",
      cv.volunteering.map((e) => (e.id === id ? { ...e, [key]: value } : e)),
    );
  }
  ...
  return (
    <>
      ...
      <Desc>
        Include community service, NGO work, club leadership, or any unpaid
        roles that demonstrate values and skills.
      </Desc>
