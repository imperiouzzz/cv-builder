...
export default function CustomStep() {
  ...
  function addEntry() {
    setCVField("customSections", [
      ...cv.customSections,
      { id: uid(), sectionTitle: "", entries: "" },
    ]);
  }
  function removeEntry(id) {
    setCVField(
      "customSections",
      cv.customSections.filter((e) => e.id !== id),
    );
  }
  function update(id, key, value) {
    setCVField(
      "customSections",
      cv.customSections.map((e) => (e.id === id ? { ...e, [key]: value } : e)),
    );
  }
  ...
  return (
    <>
      ...
      <Desc>
        Add any additional sections — Certifications, Awards, Publications,
      ...
      >
        ...
        >
          Review your CV in the live preview panel on the right. Click{" "}
          <strong>Finish ✓</strong> below to complete, or{" "}
          <strong>Export PDF</strong> in the toolbar to download now.
        </p>
        <div
          style={{
            display: "inline-flex",
