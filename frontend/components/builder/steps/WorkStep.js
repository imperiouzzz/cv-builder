...
export default function WorkStep() {
  ...
  function removeEntry(id) {
    setCVField(
      "workExp",
      cv.workExp.filter((e) => e.id !== id),
    );
  }
  function updateEntry(id, key, value) {
    setCVField(
      "workExp",
      cv.workExp.map((e) => (e.id === id ? { ...e, [key]: value } : e)),
    );
  }
  ...
  return (
    <>
      ...
      <Desc>
        Start with your most recent position. Use bullet points for
        achievements.
      </Desc>
          ...
          >
            <Grid>
              ...
              <Field label="Company / Organisation">
                <Input
                  ...
                  value={e.company}
                  onChange={(ev) =>
                    updateEntry(e.id, "company", ev.target.value)
                  }
                />
              </Field>
            </Grid>
            ...
            <Field label="Key Achievements / Responsibilities" span="full">
              <Textarea
                ...
                value={e.description}
                onChange={(ev) =>
                  updateEntry(e.id, "description", ev.target.value)
                }
                style={{ minHeight: 100 }}
              />
            </Field>
