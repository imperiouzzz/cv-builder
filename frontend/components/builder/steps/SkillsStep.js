...
const SUGGESTED = [
  ...
  "Problem Solving",
  "Leadership",
].map((name) => ({ id: uid(), name, category: "technical" }));
...
export default function SkillsStep() {
  ...
  function addSkill(name) {
    ...
    if (
      !trimmed ||
      cv.skills.find((s) => s.name.toLowerCase() === trimmed.toLowerCase())
    )
      return;
    setCVField("skills", [
      ...cv.skills,
      { id: uid(), name: trimmed, category: "technical" },
    ]);
    setInput("");
  }
  ...
  const alreadyAdded = (name) =>
    cv.skills.some((s) => s.name.toLowerCase() === name.toLowerCase());

  return (
    <>
      <StepHeader title="Skills" step={7} total={9} />
      <Desc>
        Add technical skills, tools, soft skills, and languages. These are
        critical for ATS keyword matching.
      </Desc>
      <TipBox>
        💡 ATS Tip: Copy skill names directly from job descriptions for the
        highest match score. Press Enter or comma to add each skill.
      </TipBox>
      ...
      {cv.skills.length > 0 ? (
        ...
        <p style={{ fontSize: 12, color: "#718096", marginBottom: 20 }}>
          No skills added yet. Type above or pick from suggestions below.
        </p>
      )}
          ...
          {SUGGESTED.map((s) => (
            <button
              ...
              style={{
                ...
                background: alreadyAdded(s.name)
                  ? "rgba(56,161,105,0.1)"
                  : "white",
                border: alreadyAdded(s.name)
                  ? "1px solid #38A169"
                  : "1px solid #E2E8F0",
