...
const DATE_RANGE_RE = new RegExp(
  `(${MONTH}\\s+\\d{4})\\s*[\\-–—to]+\\s*((${MONTH}\\s+\\d{4})|present|current|now|\\d{4})`,
  "i",
);
const YEAR_RANGE_RE =
...
function extractDates(block) {
  ...
  if (r)
    return {
      ...
      end: /^(present|current|now)$/i.test(r[2])
        ? "Present"
        : toTitleDate(r[2]),
    };
...
export function parseCV(raw) {
  ...
  const liM = fullText.match(
    /(?:https?:\/\/)?(?:www\.)?linkedin\.com\/in\/([\w\d\-]+)/i,
  );
  if (liM) cv.linkedin = "https://linkedin.com/in/" + liM[1];

  const ghM = fullText.match(
    /(?:https?:\/\/)?(?:www\.)?github\.com\/([\w\d\-]+)/i,
  );
  if (ghM) cv.github = "https://github.com/" + ghM[1];

  // ── 2. Name ────────────────────────────────────────────────
  const SKIP_NAME =
    /^(summary|profile|objective|education|experience|skills|contact|references|projects|about|cv\b|resume)/i;
  const nameLine = nonEmpty.slice(0, 8).find((l) => {
    ...
    return words.every((w) =>
      w
        .split("-")
        .every((p) => /^[A-Z][a-z'\.]*$/.test(p) || /^[A-Z]{2,}$/.test(p)),
    );
  });
  ...
  if (sumLines.length)
    cv.summary = sumLines
      ...
      .substring(0, 500);
  if (!cv.summary) {
    const prose = (sections.header || []).filter(
      (l) => l.length > 60 && /[a-z]/.test(l) && !/@/.test(l),
    );
    if (prose.length)
      cv.summary = prose
        .join(" ")
  ...
  // ── 5. Education ──────────────────────────────────────────
  const DEGREE_RE =
    /\b(ph\.?d|d\.?sc|m\.?sc?|m\.?phil|b\.?sc?|b\.?a\.?|m\.?a\.?|m\.?ba|hnd|b\.?eng?|b\.?tech|diploma|certificate|associate|bachelor|master|doctor|wassce|w\.a\.s\.s\.c\.e|sssce)\b/i;
  const INST_RE =
    /\b(university|college|school|institute|polytechnic|knust|ug\b|ucc|uew|uhas|academy)\b/i;

  if (sections.education) {
    const eduLines = sections.education.filter((l) => l.length > 2);
    const entries = splitByPredicate(
      eduLines,
      (l, i) => i > 0 && (DEGREE_RE.test(l) || l.length === 0),
    );
    entries.forEach((entry) => {
      ...
      if (
        locM &&
        locM[0].length < 60 &&
        !/university|school|college|science|technology/i.test(locM[0])
      ) {
      ...
      if (/expected\s+to\s+(graduate|finish|complete)/i.test(block)) {
        ...
        const startLines = nb.filter(
          (l) => /\d{4}/.test(l) && !/expected/i.test(l),
        );
      ...
      const gpaM = block.match(
        /(?:gpa|cgpa|grade|class)[:\s]*([\d.]+\s*\/\s*[\d.]+|first class|second class upper|second class lower|third class|distinction|merit|pass)/i,
      );
      if (gpaM) edu.gpa = clean(gpaM[1]);

      const achM = block.match(
        /(dean'?s list|best student|award|honour|distinction|scholarship)[^\n]*/i,
      );
  ...
  // ── 6. Work experience ────────────────────────────────────
  const TITLE_WORDS =
    /\b(engineer|developer|analyst|manager|designer|officer|intern|director|consultant|coordinator|specialist|associate|lead|head|executive|assistant|technician|administrator|accountant|researcher|programmer|architect|scientist)\b/i;
  ...
  if (sections.work) {
    ...
    entries.forEach((entry) => {
      ...
      if (/\|/.test(nb[0])) {
        ...
        job.company = clean(nb[0].substring(pipeIdx + 1))
          .replace(/^\s*,\s*|\s*,\s*$/g, "")
          .trim();
      ...
      const locM = block.match(
        /\b([A-Z][a-zA-Z\s]+),\s*([A-Z][a-zA-Z\s]+(?:Ghana|Nigeria|Region|UK|US|USA|Africa)?)/,
      );
        ...
        .filter(
          (l) =>
            ...
            (l.length > 40 &&
              !isJobHeader(l) &&
              !DATE_ONLY.test(l) &&
              !/^(key responsibilities|at bsystems|during my|this experience|this internship)/i.test(
                l,
              )),
        )
        .slice(0, 12)
  ...
  if (sections.skills) {
    ...
    skillLines.forEach((line) => {
      ...
      const skillName =
        dashIdx > 0 ? stripped.substring(0, dashIdx).trim() : stripped;
      if (skillName.length < 2 || skillName.length > 50) return;
      if (/^(technical|soft|skills?|abilities|hard skills)/i.test(skillName))
        return;
      const final = skillName.replace(/\s*\(.*?\)/, "").trim();
      if (
        final.length >= 2 &&
        /[a-zA-Z]/.test(final) &&
        !cv.skills.find((s) => s.name === final)
      ) {
  ...
  if (sections.projects) {
    ...
    splitByBlankLines(projLines).forEach((entry) => {
      ...
      const techM = block.match(
        /(?:tech|stack|built with|tools?|using)[:\s]*([^\n]+)/i,
      );
  ...
  if (volSrc.some((l) => l.length > 3)) {
    ...
    splitByPredicate(
      ...
      vol.role = clean(nb[0]).substring(0, 80);
      const atM = block.match(/\s+at\s+(.+)$/i);
      if (atM) vol.organization = clean(atM[1]).substring(0, 80);
      const d = extractDates(block);
      vol.startDate = d.start;
      vol.endDate = d.end;
      const desc = nb
        .slice(1)
        .filter((l) => l.length > 10)
        .slice(0, 5);
      if (desc.length) vol.description = desc.join(" ").substring(0, 400);
      if (vol.role || vol.organization) cv.volunteering.push(vol);
    });
  }

  // ── 10. References ───────────────────────────────────────
  if (sections.references) {
    const refLines = sections.references.filter((l) => l.length > 2);
    splitByBlankLines(refLines).forEach((entry) => {
      const block = entry.join("\n");
      const ref = {
        id: uid(),
        name: "",
      ...
      const lines = entry.filter((l) => l.length > 0);
      if (lines.length) ref.name = clean(lines[0]).substring(0, 80);
      const titleM = block.match(
        /(?:professor|dr\.?|mr\.?|mrs\.?|ms\.?|director|manager|head|chair|dean)\s*[\w\s]*/i,
      );
      if (titleM) ref.title = clean(titleM[0]).substring(0, 80);
      const compM = block.match(
        /\b(university|college|school|company|firm|organization|ministry|department)\s*[\w\s]*/i,
      );
      if (compM) ref.company = clean(compM[0]).substring(0, 80);
      const emailM = block.match(/[\w.+\-]+@[\w\-]+\.[a-zA-Z]{2,6}/);
      if (emailM) ref.email = emailM[0];
      const phoneM = block.match(
        /(?<!\d)(\+?[\d][\d\s\-\.\(\)]{6,17}[\d])(?!\d)/,
      );
      if (phoneM) ref.phone = phoneM[1].trim();
      if (ref.name) cv.references.push(ref);
    });
  }

  // ── 11. Custom sections ──────────────────────────────────
  if (sections.custom) {
    const customLines = sections.custom.filter((l) => l.length > 2);
    splitByBlankLines(customLines).forEach((entry) => {
      const block = entry.join("\n");
      const custom = { id: uid(), title: "", content: "" };
      custom.title = clean(entry[0] || "").substring(0, 80);
      const cont = entry
        .slice(1)
        .filter((l) => l.length > 5)
        .slice(0, 10);
      if (cont.length) custom.content = cont.join("\n").substring(0, 800);
      if (custom.title && custom.content) cv.customSections.push(custom);
    });
  }
  ...
  if (cv.github) filled.push("github");
  if (cv.summary) filled.push("summary");
  if (cv.education.length) filled.push("education");
  if (cv.workExp.length) filled.push("work");
  if (cv.skills.length) filled.push("skills");
  if (cv.projects.length) filled.push("projects");
  if (cv.volunteering.length) filled.push("volunteering");
  if (cv.references.length) filled.push("references");
  if (cv.customSections.length) filled.push("custom");
